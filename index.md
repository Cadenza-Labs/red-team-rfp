...
- **Models:** Examples must be generated by (possibly fine-tuned) open-weight models. We encourage proposals using mid-size to large (e.g., Qwen3.5-27B to OSS 120B). Very large models (e.g., Kimi K2.5) are also allowed, but our capacity to run model organisms at that scale during the Blue Team phase is limited. If you plan to use a very large model, we recommend reaching out in advance. In general, we advise including a smaller-model fallback (e.g., Qwen3.5-27B) in your proposal.
